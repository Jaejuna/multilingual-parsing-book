...
17. [데이터 품질을 모델 정확도로 측정하기](#17-데이터-품질을-모델-정확도로-측정하기)
18. [유사도 매칭 — exact 매칭이 놓치는 것 잡기](#18-유사도-매칭--exact-매칭이-놓치는-것-잡기)
19. [렉시콘 위에서 추론하기](#19-렉시콘-위에서-추론하기)
20. [이종 소스들을 하나의 코퍼스로 병합하기](#20-이종-소스들을-하나의-코퍼스로-병합하기)
...
→ 실행 가능: [`snippets/glossary-matching/substring_match.py`](./snippets/glossary-matching/substring_match.py),
...
플랫 용어 리스트로는 어색하지만 **trie** 로는 자연스러운 작업이 둘 있다: 타입어헤드
(`cool` 로 시작하는 모든 용어)와 공백 없는 스크립트의 최장 매칭 분절(각 위치에서 가장 긴
사전 용어 찾기). [`prefix_index.py`](./snippets/glossary-matching/prefix_index.py) 가 바로 그
trie 다 — Aho-Corasick 이 그 위에 세워지는 바로 그 구조에서 failure link 만 뺀 것이라, 더
부드러운 입문으로 읽힌다.
...
여러 어노테이터, 혹은 규칙 태거 + 모델로부터 스팬이 들어오기 시작하면 서로 겹치는데, 두
종류의 겹침은 정반대를 뜻한다: 같은 라벨끼리의 겹침은 하나의 멘션이 둘로 쪼개진 것이고
(병합), 다른 라벨끼리의 겹침은 진짜 불일치다(드러내되, 절대 병합하지 않음).
[`merge_spans.py`](./snippets/nlu/merge_spans.py) 는 스팬을 한 번 정렬하고 훑는 sort-and-sweep
한 번으로 둘 다 처리한다(O(n log n), 전쌍 비교 아님) — 캘린더와 게놈 구간에 쓰이는 바로 그
interval 패턴을, 여기서는 어노테이션 데이터를 정리하는 데 쓴다.

그리고 여러 slot 으로부터 발화를 합성할 때, 순진한 전체 데카르트 곱은 대부분 쓰레기다(같은
엔티티가 두 slot 에, 도메인이 금지하는 쌍). [`constraint_expand.py`](./snippets/nlu/constraint_expand.py)
는 **백트래킹** 으로 *유효한* 조합만 열거한다 — 각 배정을 slot 별로 하나씩 쌓아가다가 제약이
깨지는 순간 서브트리 전체를 가지치기하는, N-퀸/스도쿠의 그 형태를 깨끗한 훈련 데이터에 적용한
것이다.
...
### 알아둘 만한 스트리밍 primitive

데이터가 메모리에 안 들어가기 시작하면 몇 가지 고전 알고리즘이 계속 등장한다; 각각은
스트림 크기와 무관하게 메모리를 일정하게 유지하고, 각각은 짧은 파일 하나다:

- **bounded heap 로 Top-K** — 전체 코퍼스를 정렬하지 않고 "어떤 용어가 가장 자주 미번역으로
  남는가?" 에 답한다. 크기 K 의 min-heap 은 O(K) 메모리, O(N log K) 시간:
  [`dataset-quality/top_terms.py`](./snippets/dataset-quality/top_terms.py).
- **정렬된 shard 의 K-way 병합** — 외부 정렬의 병합 절반: RAM 에서 이어붙일 수 없는 shard 들을
  shard 당 슬롯 하나짜리 heap 으로 합친다([`scale/merge_shards.py`](./snippets/scale/merge_shards.py)).
- **reservoir sampling** — 스트림 길이를 미리 모른 채, 단일 패스로 균등한 spot-check 샘플을
  뽑는다([`scale/reservoir_sample.py`](./snippets/scale/reservoir_sample.py)).
- **슬라이딩 윈도우 + 투 포인터** — 전역 평균 대신 국소 품질: 롤링 윈도우 평균(스텝당 O(1))은
  품질이 떨어지는 구간을 국소화하고, 투 포인터 스캔은 결함 예산 안에서 가장 긴 구간을 찾는다
...
매처가 배포되고 나면, 결정은 자원 문제가 되고, 그중 둘은 통계가 아니라 교과서적 최적화다.
어떤 양이 손잡이에 대해 *단조* 일 때 — 발화하는 fuzzy 매칭 수는 임계값이 오를수록 줄기만 한다 —
리뷰 예산을 충족하는 가장 낮은 임계값은 스캔이 아니라 **답에 대한 이진 탐색** 이다
([`threshold_search.py`](./snippets/experiments/threshold_search.py) 는 불변식이 보이도록
`lower_bound`/`upper_bound` 도 함께 출력한다). 그리고 고정된 노력 예산 아래에서 가장 많은
커버리지 가치를 사기 위해 *어떤* 세그먼트를 리뷰할지 고르는 것은 **0/1 배낭(knapsack)** 이고,
정확한 단어 수 배치를 가장 적은 세그먼트로 맞추는 것은 **동전 거스름(coin change)** 이다
([`budget_allocation.py`](./snippets/experiments/budget_allocation.py)) — 둘 다 위치가 아니라
예산으로 인덱싱되는 동적 계획법이다.

## 17. 데이터 품질을 모델 정확도로 측정하기

다른 모든 장은 데이터를 제품으로 다룬다. 이 장은 그것을 소비하는 것 — 모델 — 과 연결한다.
"라벨을 정제하라"는 가격을 매길 수 있기 전까지는 조언일 뿐이다.
[`data_quality_impact.py`](./snippets/data-model/data_quality_impact.py) 는 실제 분류기를
훈련한다 — 표준 라이브러리 multinomial Naive Bayes, 그냥 로그 공간에서 스무딩된 단어
카운트 — 알려진 양의 라벨 노이즈로 오염된 훈련 데이터 위에서, 그리고 그것을 **깨끗한**
테스트 셋에 대해 채점한다:
...
형태가 곧 교훈이고, 그것은 "노이즈가 많을수록 선형적으로 나빠짐"이 아니다. Naive Bayes 는
훈련 셋 전체에 걸쳐 카운트를 집계하므로, 적당한 라벨 노이즈는 대부분 평균으로 상쇄된다 —
정확도는 약 40% 까지 거의 움직이지 않는다 — 그러다 라벨이 50% 틀림에 가까워지면 chance 를
향해 붕괴한다. 데이터 품질 예산은 실재하지만 미묘하다: 강건한 모델은 어느 정도 노이즈를
견디고, 이 곡선이 그게 얼마인지를 정확도 포인트로 말해준다. (테스트 셋은 절대 오염되지
않는다 — 우리는 *훈련* 데이터가 진실에 대해 채점되는 모델을 얼마나 망치는지 측정한다.)
또한 이 책에 작지만 진짜인 ML 모델을 들여, 데이터→모델 연결이 손짓으로 끝나지 않게 한다.

## 18. 유사도 매칭 — exact 매칭이 놓치는 것 잡기

4장의 매처는 exact 다: `cooldown` 은 `cooldown` 에만 매칭하고 그 외엔 아무것도 매칭하지
않는다. 실제 입력은 `cool-down`, `cooldwn`(오타), `cooldowns`(복수형) 을 가져온다. 그것들을
회복하는 수는 문자열 비교를 멈추고 *표현(representation)* 을 비교하는 것이다 — 각 용어를
벡터로 바꾸고 cosine 유사도를 측정한다.
[`fuzzy_match.py`](./snippets/matching-similarity/fuzzy_match.py) 는 char n-gram TF-IDF
로 그것을 한다: 모델도, 의존성도 없지만, 진짜 벡터 공간이다.
...
exact 매칭은 첫 row 에서만 발화한다. fuzzy 는 변종들을 회복하면서도 무관한 단어는 여전히
거부한다. 왜 word embedding 이 아니라 char n-gram 인가: 오타와 형태 변화에 강건하고,
사전훈련 모델이 필요 없으며, 이 책이 다루는 혼재된 스크립트에서도 살아남는다 — *표면*
유사도를 잡되 *의미* 는 잡지 못한다는 비용을 치르고. `big` 과 `large` 는 공유 문자가 없는
동의어라, 이건 그 둘을 잇지 못한다. 거기가 진짜 embedding 이 그 값어치를 하는 곳이다.
당신이 가진 문제가 어느 쪽인지 알 것. 이것은 규칙(#4)에서 학습된 표현으로 가는 on-ramp 다.

유사도 점수가 아니라 *딱 떨어지는* 답이 필요할 때 — "이 후보를 실제 용어와 한 번의 편집
거리 안일 때만 받아들여라" — [`edit_distance.py`](./snippets/matching-similarity/edit_distance.py)
는 고전적인 two-row DP 로 Levenshtein 거리를 계산하고, 거리가 예산을 넘는 것이 확정되는
즉시 중단하는 *bounded* 변형도 함께 제공한다(그래서 큰 용어 리스트를 선별해도 비용이 싸게
유지된다). cosine 은 순위를 매기고, edit distance 는 게이트를 건다.

edit distance 는 시퀀스 DP 한 패밀리의 하나다.
[`sequence_align.py`](./snippets/matching-similarity/sequence_align.py) 는 코퍼스 작업에서
거듭 등장하는 나머지 둘을 더한다: **LCS**(가장 긴 순서 보존 공통 토큰 구간 — `diff` 의
backbone 이라, 한 리비전이 무엇을 바꿨는지 볼 수 있다)와 **LIS**(품질 점수 스트림에서 가장 긴
비후퇴 구간). 같은 표를-만들고-그것을-걷는 형태, 다른 질문.

## 19. 렉시콘 위에서 추론하기

11장은 개념 그래프를 만들었지만 직접 연결된 `broader` 엣지만 읽었다. 지식 그래프를 가질
값어치 있게 만드는 질문들은 transitive 하다: `loot` 의 *모든* 조상은 무엇인가, `object` 아래
무엇이 들어가는가, 그리고 한 문장은 어떤 개념들을 언급하는가?
[`build_lexicon.py`](./snippets/knowledge-graph/build_lexicon.py) 가 이제 그것들에 답한다.
...
- `ancestors`/`descendants` 는 `broader` 체인을 **transitive** 하게 걷는다, cycle 가드와
  함께 — 그 transitive 조상 관계가 한 개념이 부모의 도메인을 상속하게 해주는 것이다
- `find_cycles()` 는 깨진 `a → b → a` 계층을 거부한다(아니면 순회가 종료되지 않는다)
- `link()` 는 **엔티티 링킹** 이다: 자유 텍스트를 용어집 표면형에 대해 스캔하고(라틴은 단어
  경계 #4, CJK 는 substring), 각각을 그 개념으로 resolve 한다, 가장 긴 표면형 우선이라
  "AI Director" 가 "AI" 를 이긴다
- `topo_order()` (`--topo`) 는 위상 정렬(Kahn 알고리즘)로 개념을 넓은 것 먼저, 좁은 것 나중
  순서로 반환한다 — 도메인을 계층 *아래로* 전파하거나 자식이 부모와 결코 모순되지 않는지
  검증할 때 필요한 그 순서다; `find_cycles()` 가 루프를 찾아내도 전순서로 degrade 한다

transitive 추론에 엔티티 링킹을 더하면 #11 의 플랫 렉시콘이 실제로 쿼리할 수 있고 실행
중인 텍스트에 연결할 수 있는 무언가로 바뀐다.

렉시콘은 일반 그래프이기도 하다 — 개념들은 위아래뿐 아니라 옆으로도 관계를 맺는다 — 그래서
"두 개념은 얼마나 떨어져 있고, 어떤 경로로 연결되나?" 가 순회 질문이 된다.
[`concept_paths.py`](./snippets/knowledge-graph/concept_paths.py) 는 그것에 세 방식으로 답하고
그것들이 서로 어긋남을 보여준다: **BFS** 는 최소 홉 경로를 찾고, **DFS** 는 도달 가능성에
답하며, **Dijkstra** 는 엣지에 가중치가 있을 때 최소 *비용* 경로를 찾는다 — 이는 흔히 BFS
경로보다 홉이 더 많다. 질문에 맞는 순회를 고를 것.

## 20. 이종 소스들을 하나의 코퍼스로 병합하기

데이터셋은 좀처럼 한 개의 깔끔한 파일이 아니다. 그것은 export 들의 폴더다 — 하나는 Excel
이 cp949 로 저장했고(#1), 하나는 `ko_KR` 헤더에 또 하나는 `ko-KR`(#2), 겹치는 키들, 그리고
같은 키가 두 파일에서 서로 다르게 번역된 것. 맹목적으로 이어붙이면 mojibake, 쪼개진 컬럼,
중복, 조용한 충돌이 나온다.
[`build_corpus.py`](./snippets/multi-source/build_corpus.py) 는 제대로 병합하고, 결정적으로
**리포트** 한다:
...
이것은 Part I 의 캡스톤이다: 인코딩 폴백(#1, utf-8-sig 후 cp949), lang 코드
정규화(#2, `ko_KR`/`KO-KR` → `ko-KR`), 더해서 병합이 필요로 하는 두 가지 —
**충돌 탐지**(같은 키 + 언어, 다른 값; 첫 소스가 이기고, 진 쪽은 리포트되며 결코 조용히
버려지지 않음)와 **provenance**(각 값이 어느 소스에서 왔는지). base `ko` 와 locale `ko-KR`
는 기본적으로 별개 컬럼으로 남는다 — 합치면 region 을 잃는다(#2 의 `zh-CN` vs `zh-TW`
트레이드오프); `--merge-base` 는 그걸 받아들일 때 둘을 접는다.

`build_corpus.py` 는 row 를 *exact* id 로 키잉한다. 같은 엔티티가 표면적 변형으로
(`Cooldown`, `cool-down`, `Cooldown `) 나타나고, 오직 간접적으로만 묶여 있을 때(두 레코드가
이름이 아니라 외부 id 를 공유), [`cluster_duplicates.py`](./snippets/multi-source/cluster_duplicates.py)
는 **union-find** 로 그것들을 묶는다: 공유된 신호 하나하나가 두 레코드를 union 하고, 남은
연결 컴포넌트가 곧 중복 제거된 엔티티가 된다 — exact-match 중복 제거가 조각으로 쪼개버리는
transitive 한 `A~B~C` 케이스를 포착한다.
...
| 실제 용어와 N 번의 편집 안일 때만 오타 허용 | [`matching-similarity/edit_distance.py`](./snippets/matching-similarity/edit_distance.py) |
| 접두사로 용어 자동완성 / 공백 없는 텍스트 분절 | [`glossary-matching/prefix_index.py`](./snippets/glossary-matching/prefix_index.py) |
| 전부 정렬하지 않고 미번역 Top-K 용어 순위 매기기 | [`dataset-quality/top_terms.py`](./snippets/dataset-quality/top_terms.py) |
| RAM 보다 큰 정렬된 코퍼스 shard 병합 | [`scale/merge_shards.py`](./snippets/scale/merge_shards.py) |
| 크기를 모르는 스트림에서 균등 spot-check 샘플 | [`scale/reservoir_sample.py`](./snippets/scale/reservoir_sample.py) |
| 겹치는 어노테이션 스팬 병합 / 라벨 충돌 표시 | [`nlu/merge_spans.py`](./snippets/nlu/merge_spans.py) |
| 개념 계층을 넓은 것 먼저 순서로 정렬 | [`knowledge-graph/build_lexicon.py`](./snippets/knowledge-graph/build_lexicon.py) `--topo` |
| transitive 하게만 연결된 중복 레코드 합치기 | [`multi-source/cluster_duplicates.py`](./snippets/multi-source/cluster_duplicates.py) |
| 개념 간 최단 vs 최저비용 경로 (BFS / DFS / Dijkstra) | [`knowledge-graph/concept_paths.py`](./snippets/knowledge-graph/concept_paths.py) |
| 리뷰 예산을 충족하는 가장 낮은 임계값 (이진 탐색) | [`experiments/threshold_search.py`](./snippets/experiments/threshold_search.py) |
| 품질 저하 구간 국소화 (슬라이딩 윈도우 / 투 포인터) | [`scale/windowed_metrics.py`](./snippets/scale/windowed_metrics.py) |
| 제약이 걸린 slot 조합 열거 (백트래킹) | [`nlu/constraint_expand.py`](./snippets/nlu/constraint_expand.py) |
| 두 토큰 리비전 diff / 가장 긴 비후퇴 구간 (LCS, LIS) | [`matching-similarity/sequence_align.py`](./snippets/matching-similarity/sequence_align.py) |
| 가장 많은 커버리지를 위해 예산 쓰기 (배낭 / 동전 거스름) | [`experiments/budget_allocation.py`](./snippets/experiments/budget_allocation.py) |
...
8. **`.lower()` 는 caseless 매칭이 아니다 (리뷰에서 발견).** 모든 매처가 `.lower()` 로
   케이스를 접었는데, 이는 라틴 밖에서 틀리다: 독일어 `STRASSE` ≠ `straße`, 터키어
   `İ`/`ı`, 그리스어 어말 시그마. 수정: 유니코드 caseless fold 인 `str.casefold()` —
   이제 모든 매처(#4, #9, #18, Aho-Corasick)에서 사용.
9. **전각/반각 변형이 조용히 누락된다 (#7).** 용어집 `AI` 는 IME·레거시 시스템이 뱉는
   전각 `ＡＩ` 나 반각 가타카나와 매칭되지 않았다. 수정: 비교 전 매치 키를 NFKC 정규화
   (`unicodedata.normalize("NFKC", s)`); 매처는 이제 NFKC + casefold 를 함께 접는다.
   NFKC 는 손실적이므로 저장/표시 원본이 아니라 매치 키에만 적용한다.
